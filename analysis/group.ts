...
export class Group {
    ...
    findEV = () => {
        ...
        return this.prices.map(price => {
            return {
                book: price.book,
                EV: averageLikelihood * Odds.fromFairLine(price.price).toPayoutMultiplier() - (1 - averageLikelihood)
            }
        })
    }
    getAveragePrice = () => {
    ...
    findRelatedGroups = (groups: Group[]) => {
        return groups.filter(group => group.stat === this.stat && group.side === this.side && compareTwoStrings(group.name, this.name) > 0.85 && group.value !== this.value)
    }
    maxEV = () => {
