...
const findEquivalentPlays = (line: Line, corpus: Line[]) =>
  corpus.filter((sampleLine) => {
    ...
    if (
      ...
      if (
        Math.abs((sampleLine as Spread).value) !==
        Math.abs((line as Spread).value)
      ) {
        return false;
      }
    }
  ...
  while (remainingLines.length) {
    ...
    const sameOutcome = matchingLines.filter((l) => {
      if (
        ...
        return (
          line.choice === l.choice &&
          (line as Spread).value === (l as Spread).value
        );
      }
    ...
    const otherOutcome = matchingLines.filter((l) => {
      ...
      if (
        ...
        return (
          line.choice !== l.choice &&
          (line as TeamTotal).value === (l as TeamTotal).value
        );
      }
      return line.choice !== l.choice;
    });
    ...
    remainingLines = remainingLines.filter(
      (l) =>
        l !== line && !group.outcome1.includes(l) && !group.outcome2.includes(l)
    );
    groups.push(group);
  }
  return groups;
...
const compareLines = (group: Group) => {
  ...
  group.outcome1.forEach((outcome1) => {
    group.outcome2.forEach((outcome2) => {
      const outcome1Probability = Odds.fromFairLine(
        outcome1.price
      ).toProbability();
      const outcome2Probability = Odds.fromFairLine(
        outcome2.price
      ).toProbability();
...
export const formatResults = async (arbOpportunities: Opportunity[]) => {
  const formatted = arbOpportunities.map((opportunity) => {
    const label = `${opportunity.outcome1.awayTeam} @ ${
      opportunity.outcome1.homeTeam
    } -${
      opportunity.outcome1 instanceof TeamTotal
        ? " " + opportunity.outcome1.side
        : ""
    ...
    const payouts = roundedStakes.map(
      (stake, idx) => stake + stake * multipliers[idx]
    );
